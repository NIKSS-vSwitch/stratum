...
::util::Status NikssChassisManager::PushChassisConfig(
  ...
  //zapisac konfiguracje do mapy vvv

  
  std::map<uint32, PortConfig> chassis_config;
  ...
  for (const auto& singleton_port : config.singleton_ports()) {
    ...
    chassis_config[port.port_id] = port;
  }
...
::util::StatusOr<std::map<uint32, NikssChassisManager::PortConfig>> NikssChassisManager::GetPortConfig() const {
  //const PortConfig* config_out = chassis_config_;
  //const PortConfig* config_out = gtl::FindOrNull(port_config_, port_id);
  /*RET_CHECK(config_out != nullptr)
